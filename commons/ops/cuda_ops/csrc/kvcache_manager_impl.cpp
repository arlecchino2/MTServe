...
std::vector<std::pair<int64_t, int64_t>> HostKVStorageImpl::get_cache_startpos_and_length(std::vector<int64_t>& user_ids) {
    int batch_size = user_ids.size();
    std::vector<std::pair<int64_t, int64_t>> result(batch_size);
    for (int seq_idx = 0; seq_idx < batch_size; seq_idx++) {
        int64_t uid = user_ids[seq_idx];
        auto it = _uid_to_length.find(uid);
        if (it != _uid_to_length.end()) {
            result[seq_idx] = std::make_pair(0, it->second);
        } else {
            result[seq_idx] = std::make_pair(0, 0);
        }
    }
    return result;
};
...
GPUKVCacheMangerImpl::~GPUKVCacheMangerImpl() {
    ...
    {
        std::unique_lock<std::mutex> lock(pending_sync_mutex_);
            for (void* ptr : pending_sync_operations) {
                if (ptr != nullptr) {
                    cudaFree(ptr);
                }
            }
            pending_sync_operations.clear();
    }
}
...
std::vector<std::pair<int32_t, int32_t>> GPUKVCacheMangerImpl::get_cache_startpos_and_length(std::vector<int64_t>& user_ids) {
    int batch_size = user_ids.size();
    std::vector<std::pair<int32_t, int32_t>> result(batch_size);
    for (int seq_idx = 0; seq_idx < batch_size; seq_idx++) {
        int64_t uid = user_ids[seq_idx];
        int32_t start_pos = 0;
        int32_t gpu_cache_length = 0;
        
        if (_uid_to_paged_cache_startpos.find(uid) != _uid_to_paged_cache_startpos.end()) {
            start_pos = _uid_to_paged_cache_startpos[uid];
            gpu_cache_length = _uid_to_paged_cache_length[uid];
        } else if (_uid_to_offloaded_length.find(uid) != _uid_to_offloaded_length.end()) {
            start_pos = _uid_to_offloaded_length[uid];
            gpu_cache_length = 0;
        }
        result[seq_idx] = std::make_pair(start_pos, gpu_cache_length);
    }
    return result;
};
...
void sync_onload_buffer_to_cache(
    ...
    for (int seq_idx = 0; seq_idx < batch_size; ++seq_idx) {
        ...
        for (int i = 0; i < num_pages; ++i) {
            ...
            global_target_pages.push_back(gpu_mgr._empty_pages.front());
    ...
    int32_t* target_page_ids_gpu;
    cudaMalloc(reinterpret_cast<void**>(&target_page_ids_gpu), global_target_pages.size() * sizeof(int32_t));
    cudaMemcpyAsync(target_page_ids_gpu, global_target_pages.data(), 
                global_target_pages.size() * sizeof(int32_t), 
                cudaMemcpyHostToDevice, gpu_mgr.scatter_stream);

    uint16_t* gpu_onload_buffer = gpu_mgr.get_cache_table_by_layer(0) + 
                                gpu_mgr.num_primary_cache_pages * gpu_mgr.page_stride;
    
    scatter_paged_kv_cache_all_layers(
        gpu_onload_buffer,                           // 连续源（起始位置）
        gpu_mgr.get_cache_table(),                     // 离散目标（完整缓存表）
        static_cast<int*>(target_page_ids_gpu),          // 页面ID映射
        gpu_mgr.num_layers,                          // 多层，与offload一致
        total_onload_length * gpu_mgr.per_token_kv_stride / sizeof(uint16_t), // 连续stride
        gpu_mgr.layer_stride,                          // 目标stride
        gpu_mgr.num_kv_heads,
        gpu_mgr.kv_headdim,
        gpu_mgr.num_tokens_per_page,
        gpu_mgr.page_stride,
        gpu_mgr.k2v_stride,
        gpu_mgr.per_token_kv_stride,
        gpu_mgr.kv_headdim,
        global_target_pages.size(),
        k_num_sms,
        gpu_mgr.scatter_stream);
    
    {
        std::unique_lock<std::mutex> lock(gpu_mgr.pending_sync_mutex_);
        gpu_mgr.pending_sync_operations.push_back(target_page_ids_gpu);
    }
}
...
void synchronize_sync_stream(
    ...
    cudaStreamSynchronize(gpu_mgr.scatter_stream);
    {
        std::unique_lock<std::mutex> lock(gpu_mgr.pending_sync_mutex_);
        for (void* ptr : gpu_mgr.pending_sync_operations) {
            if (ptr != nullptr) {
                cudaFree(ptr);
            }
        }
        gpu_mgr.pending_sync_operations.clear();
    }
};
